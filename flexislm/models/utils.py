...
def load_train_flexislm_model_and_tokenizer(
    ...
    config = ParallelS2SConfig(
        ...
        qwen25o_encoder_path=getattr(model_args, 'qwen25o_encoder_path', None),
        qwen25o_encoder_config_path=getattr(model_args, 'qwen25o_encoder_config_path', None),
        ...
        use_depth_transformer=getattr(model_args, 'use_depth_transformer', False),
        depth_transformer_num_layers=getattr(model_args, 'depth_transformer_num_layers', 6),
        depth_transformer_dim=getattr(model_args, 'depth_transformer_dim', 1024),
        depth_transformer_num_heads=getattr(model_args, 'depth_transformer_num_heads', 16),
        depth_transformer_dim_feedforward=getattr(model_args, 'depth_transformer_dim_feedforward', 4096),
        num_acoustic_quantizers=getattr(model_args, 'num_acoustic_quantizers', 11),
        acoustic_codebook_size=getattr(model_args, 'acoustic_codebook_size', 1024),
        acoustic_loss_weight=getattr(model_args, 'acoustic_loss_weight', 10.0),
        # Flow-matching depth head (predicts FlexiCodec decoder_latent)
        use_flow_matching_depth=getattr(model_args, 'use_flow_matching_depth', False),
        flow_matching_sigma=getattr(model_args, 'flow_matching_sigma', 1e-5),
        flow_matching_time_scheduler=getattr(model_args, 'flow_matching_time_scheduler', 'cos'),
        flow_matching_cfg_scale=getattr(model_args, 'flow_matching_cfg_scale', 0.2),
        decoder_latent_dim=getattr(model_args, 'decoder_latent_dim', 512),
        flow_matching_prev_latents=getattr(model_args, 'flow_matching_prev_latents', 4),
        flow_matching_use_dim_schedule_shift=getattr(model_args, 'flow_matching_use_dim_schedule_shift', False),
        flow_matching_base_dim=getattr(model_args, 'flow_matching_base_dim', 4096),
        flow_matching_effective_dim=getattr(model_args, 'flow_matching_effective_dim', 0),
        flow_matching_shift_latent=getattr(model_args, 'flow_matching_shift_latent', False),
